...
beta_portfolios = beta_portfolios/100  # Convert to decimal returns
...
avg_returns = beta_portfolios[portfolio_cols].mean()

# Get risk-free rate and market return (annualized)
rf_rate = ff_factors["RF"].mean() 
market_return = (ff_factors["MKT_RF"].mean() + ff_factors["RF"].mean()) 
...
for idx, col in enumerate(portfolio_cols):
    ...
    cumulative_returns = (1 + beta_portfolios[col]).cumprod()
    ...
    ax.plot(
        ...
        cumulative_returns,
        linewidth=2,
        label=f"Portfolio {idx+1} (β={betas[idx]:.2f})",
        color=colors[idx],
        alpha=0.8,
    )
...
ax.set_title(
    "Buy-and-Hold Returns: Beta-Sorted Portfolios", fontsize=14, fontweight="bold"
)
...
- Fama and French use the GRS test statistic to jointly test whether the alphas ($\alpha_i$​) of all the test portfolios are zero.
