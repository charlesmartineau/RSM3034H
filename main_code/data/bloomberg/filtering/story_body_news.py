...
output_dir = Path("H:/users_fac/martineau_charles/data/restricted/bloomberg_news_aggregation/us/")

# load crsp file
headlines = pl.read_parquet(output_dir / "bloomberg_daily_aggregation_headlines_crsp_ticker.parquet")
...
for year in files_by_year:
    ...
    for file in tqdm(files_by_year[year], desc="Processing Bloomberg News files", unit="file"):
        ...
        df = (
            ...
            .filter(
                (pl.col("language_string") == "ENGLISH") &
                (pl.col("story_id").is_in(story_id))
            )
            #.with_columns(
            #    pl.col("capture_time").dt.convert_time_zone("America/New_York")
            #)
        ).select(["capture_time", "time_of_arrival", "event", "story_id", "derivedtickers", "derivedtopics", "headline", "body"])
    ...
    df_news.write_parquet(output_dir / f"bloomberg_story_crsp_ticker_{year}.parquet")
